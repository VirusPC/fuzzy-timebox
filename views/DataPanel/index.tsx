...
const DataConsole: React.FC<{}> = observer(() => {
  ...
  const onDataSelectChanged = useCallback((name: string) => {
    ...
    axios.get(url).then((response) => {
      const rawData = (Papa.parse(response.data, { skipEmptyLines: true }).data as RawData).slice(0, 10);
  ...
  const onApply = useCallback(() => {
    ...
    setTimeout(() => {
      autorun(() => {
        ...
        dataStore.apply();
        dataStore.setIsComputing(false);
      });
  ...
  return (<div className={styles["console"]}>
    ...
    <div className={styles["param-block"]}>
      ...
      <Select className={styles["select2"]} disabled={!datasetName || isLoading} defaultValue={"Select Dataset"} value={aggregationAttrPos < 0 ? void 0 : headers[aggregationAttrPos]} onChange={onAggregationAttrChanged}>
        {headers.map((attr, i) => (<Option key={attr} value={i}>{attr}</Option>))}
      </Select>
    </div>
    <div className={styles["param-block"]}>
      <Title level={5} className={styles["title"]}>Time Attribute:</Title>
      <Select className={styles["select2"]} disabled={!datasetName || isLoading} defaultValue={"Select Dataset"} value={timeAttrPos < 0 ? void 0 : headers[timeAttrPos]} onChange={onTimeAttrChanged}>
        {headers.map((attr, i) => (<Option key={attr} value={i}>{attr}</Option>))}
      </Select>
    </div>
    <div className={styles["param-block"]}>
      <Title level={5} className={styles["title"]}>Value Attribute:</Title>
      <Select className={styles["select2"]} disabled={!datasetName || isLoading} defaultValue={"Select Dataset"} value={valueAttrPos < 0 ? void 0 : headers[valueAttrPos]} onChange={onValueAttrChanged}>
        {headers.map((attr, i) => (<Option key={attr} value={i}>{attr}</Option>))}
      </Select>
    </div>
    <div className={styles["param-block"]}>
