...
//   toString() {
//     return `VolKDTree (${this._boundingBox.join(", ")}) ${
//       this._children.length
//         ? `${this.dividedBy} {
//   ${this._children[0].toString()}
//   ${this._children[1].toString()}
// }`
//         : ""
//     }`;
//   }
...
//     );
//     this._volume = Math.abs(
//       (this._boundingBox[1] - this._boundingBox[0]) *
//         (this._boundingBox[3] - this._boundingBox[2])
//     );
//     if (!avoidRecursive) {
//       this._tryDivide();
//     }
...
//     const xDivide = (this._boundingBox[0] + this._boundingBox[1]) / 2; // Assume time are equally distributed in space
//     const yMinDivide = selectK(
//       this._data.map((x) => x[2]),
//       Math.floor(this._data.length / 2)
//     );
//     const yMaxDivide = selectK(
//       this._data.map((x) => x[3]),
//       Math.floor(this._data.length / 2)
//     );
//     const slopeDivide = selectK(
//       this._data.map((x) => x[4]),
//       Math.floor(this._data.length / 2)
//     );
//     // console.log(xDivide, yMinDivide, yMaxDivide, slopeDivide);
//     const xChildren = [
//       splitLines(this._data, this._boundingBox[0], xDivide, 0),
//       splitLines(this._data, xDivide, this._boundingBox[1], 0),
//     ];
//     const yMinChildren = [
//       splitLines(this._data, this._boundingBox[2], yMinDivide, 1),
//       splitLines(this._data, yMinDivide, this._boundingBox[3], 1),
//     ];
//     const yMaxChildren = [
//       splitLines(this._data, this._boundingBox[2], yMaxDivide, 1),
//       splitLines(this._data, yMaxDivide, this._boundingBox[3], 1),
//     ];
//     const slopeChildren = [
//       splitLines(this._data, -Infinity, slopeDivide, 2),
//       splitLines(this._data, slopeDivide, Infinity, 2),
//     ];
...
//       case xCost[2]:
//         this._dividedBy = "x";
//         this._children = xCost.slice(0, 2);
//         break;
//       case yMinCost[2]:
//         this._dividedBy = "yMin";
//         this._children = yMinCost.slice(0, 2);
//         break;
//       case yMaxCost[2]:
//         this._dividedBy = "yMax";
//         this._children = yMaxCost.slice(0, 2);
//         break;
//       case slopeCost[2]:
//         this._dividedBy = "slope";
//         this._children = slopeCost.slice(0, 2);
//         break;
//     }
...
//       let order = [...this._children];
//       if (inBox(this._children[1]._boundingBox, point)) {
//         swap(order[0], order[1]);
//       }
//       for (let i = 0; i < 2 && heap.size() < k; i++) {
//         if (
//           heap.size() < k ||
//           closestDist(order[i]._boundingBox, point) < heap.top()
//         )
...
//         const dist = dist2Seg(this._data[i], point);
//         if (heap.size() < k || heap.top() > dist) {
//           if (heap.size() === k) heap.pop();
//           heap.push({ v: dist, d: this._data[i] });
...
//     if (largestDist(this._boundingBox, point) <= r) {
//       concat(ret, this._data, visit);
//       return;
//     }
//     if (closestDist(this._boundingBox, point) > r) {
//       return;
//     }
//     if (this._children.length) {
//       this._children[0].findRNN(point, r, ret, visit);
//       this._children[1].findRNN(point, r, ret, visit);
//     } else {
...
//     }
//     if (this._children.length) {
//       this._children[0].findRange(lo, hi, ret, visit);
//       this._children[1].findRange(lo, hi, ret, visit);
//     } else {
//       this._data.forEach((d) => {
...
// //_region MOM algorithm to find median
...
//       raw,
//     }));
//     this._treeRef = new RBush3D(16);
//     this._treeRef.load(this._data);
//   }
...
//     var queue = new Queue(undefined, this._compareDist);

//     while (node) {
//       for (i = 0; i < node.children.length; i++) {
//         child = node.children[i];
//         dist = this._boxDist(x, y, child);
...
//     this._repTree = { children: [] };
//     let stack = [[this._treeRef.data, this._repTree, 0]];
...
//         const points = this._generatePoints(
//           indexes,
//           lines,
//           treeNode,
...
//     return this._repTree.indexes;
//   }
//   _generatePoints(indexes, lines, box, sampleNumber) {
...
//   _boxDist(x, y, box) {
//     var dx = this._axisDist(x, box.minX, box.maxX),
//       dy = this._axisDist(y, box.minY, box.maxY);
//     return dx * dx + dy * dy;
//   }
...
export function lineRectCollide(line: {
  x1: number,
  x2: number,
  y1: number,
  y2: number,
...
export function lineRectCollide2(line: {
  x1: number,
  x2: number,
  y1: number,
  y2: number,
...
export function lineSegmentsCollide(p0: Point2D, p1: Point2D, p2: Point2D, p3: Point2D) {
...
//     if (yy < y) return brensenhamHigh(xx, yy, x, y, hashmap, slope, screenWidth, screenHeight);
//     return brensenhamHigh(x, y, xx, yy, hashmap, slope, screenWidth, screenHeight);
//   } else {
//     if (xx < x) return brensenhamLow(xx, yy, x, y, hashmap, slope, screenWidth, screenHeight);
//     return brensenhamLow(x, y, xx, yy, hashmap, slope, screenWidth, screenHeight);
//   }
// }

// function brensenhamLow(x0: number, y0: number, x1: number, y1: number, hashmap: {[hash: number]: number}, slope: number, screenWidth: number, screenHeight: number) {
...
//       else hashmap[x * screenHeight + y] += delta;
//     }
//     if (D > 0) {
//       y += yi;
//       D -= 2 * dx;
...
// function brensenhamHigh(x0: number, y0: number, x1: number, y1: number, hashmap: number[], slope: number, screenWidth: number, screenHeight: number) {
//   let delta = Math.min(1, Math.abs(1 / slope));
//   if (!isFinite(delta) || isNaN(delta)) delta = 1;
//   let dx = x1 - x0;
//   let dy = y1 - y0;
 ...
 * 
 * @param ls 
 * @param {number[][][]} hashmap x/y/lineId 
 * @param lineId 
 * @param slope 
 * @returns 
 */
